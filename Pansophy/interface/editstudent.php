...
if(!isset($_POST['submit'])){	
	...
	if($f2 == 1) $f2 = 'checked';
	if($f3 == 1) $f3 = 'checked';
        if($ferpaCheck == 1) $ferpaCheck = 'checked';
}
	...
	if(isset($_POST['f3']) && strcmp($_POST['f3'],'on') == 0) $f3 = 1;
        if(isset($_POST['ferpaCheck']) && strcmp($_POST['ferpaCheck'],'on') == 0) {
$ferpaCheck = 1;/*$ferpa = $_POST['ferpa'];*/
}
   ...
   if(get_magic_quotes_gpc()){
	...
	$edits['Field2'] = $f2;
	$edits['Field3'] = $f3;
        $edits['ferpaCheck'] = $ferpaCheck;

	// make modifications and go back to student page
	
if(($ferpaCheck == 0)&&(!empty($ferpa))){
//$studentId = $_POST['studentId'];
//$_POST['submit'] = 0;
			echo '<p class="errortext">Error: You must Check the FERPA box if you want to add FERPA flag.</p>';
