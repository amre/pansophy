...
function getIssuedContacts($issueID)
{
$dam = new DataAccessManager();
$contactstring="";
$contacts=$dam->getIssuesContacts($issueID);
if ( sizeof( $contacts ) == 0 ) {
	$contactstring=$contactstring.'ERROR: There are no contacts for this issue.  Please contact your system administrator.';
}
for($i=0; $i<sizeof($contacts); $i++){
	...
	$contactstring=$contactstring.'<table class="contact"><td class="left">'.readableDateAndTime($contact['DateCreated']).'</td>
	<td class="right" nowrap>
	<a href="mailto:'.$Creator['Email'].'">'.$Creator['FirstName'].' '.$Creator['LastName'].'</a>';
	if($dam->userCanModifyContact('', $contact['ID'])){
		$contactstring=$contactstring.' <a href="./editcontact.php?id='.$contact['ID'].'"><b>[Edit]</b></a>';
	}
	if($dam->userCanDeleteContact('', $contact['ID'])){
		$contactstring=$contactstring.' <a href="./deletecontact.php?ID='.$contact['ID'].'"><b>[Delete]</b></a>';
	}
	$contactstring=$contactstring.'</td></tr>
	...
	if(!empty($files)){
		$contactstring=$contactstring.'<tr><td><b>Attached Files</b></td></tr>';
		for($j=0; $j<count($files); $j++){
			
			// get and print filename
			$filename = $dam->getAttachedFileName($files[$j]);
			$contactstring=$contactstring.'<tr><td>'.$filename.'</td>';
			$contactstring=$contactstring.'<td class="right">';
			
			// if user has permission, print download and delete links
			if($dam->userCanDownloadFile('',$files[$j])){
				$contactstring=$contactstring.'<a href="./viewissue.php?id='.$issueID.'&fileop=download&fileid='.$files[$j].'" target="_blank">[Open]</a>';
			}
			if($dam->userCanDeleteFile('',$files[$j])){
				$contactstring=$contactstring.'<a href="./viewissue.php?id='.$issueID.'&fileop=delete&fileid='.$files[$j].'">[Delete]</a>';
			}
			$contactstring=$contactstring.'</td></tr>';
		}
		$contactstring=$contactstring.'<tr><td height="5"></td></tr>';
	}
	// done showing files
		
		
	$contactstring=$contactstring.'<tr><td class="left"><i>Students involved:</i> ';
	$emailStudents = "";
	for($j=0; $j<count($Students); $j++){
		if(!empty($Students[$j])){
			...
			$contactstring=$contactstring.'<a href="./viewstudent.php?id='.$Students[$j].'" TARGET="Main">'.$FirstName.' '.$LastName.'</a>';
		...
		else $contactstring=$contactstring.' ERROR';	
	}
	$contactstring=$contactstring."</td>
	...
	$contactstring=$contactstring.'</tr></table><p>';
}
